(function(ns)
{
	ns.ComEventos = {
		template: `<div id="com-siat-puntos-venta">
			...
			<div class="panel panel-default car border shadow mb-2" v-for="(item, index) in items">
				<div class="panel-body card-body">
					<div class="row">
						...
						<div class="col-12 col-sm-4">
							<div><b>Codigo Recepcion:</b> {{ item.codigo_reception }}</div>
						...
						<div class="col-12 col-sm-2">
							<span class="badge" v-bind:class="{'bg-success': item.status == 'OPEN', 'bg-danger': item.status == 'CLOSED', 'bg-warning': item.status == 'VOID'}">
											...
											<div class="list-group">
												<div class="list-group" v-for="(pkg, pkgi) in item.packages">
													<div>
													    ID: {{ pkg.id }}
													    ESTADO: {{pkg.status}}
													    ESTADO RECEPCION: {{ pkg.reception_status }}
													</div>
													<div></div>
													<div>{{ pkg.data }}</div>
												</div>
					...
					<form ref="formnew" class="modal-content" novalidate>
						...
						<div class="modal-body">
							...
							<div class="mb-3">
								...
								<select class="form-control form-select no-select2" required v-model="evento.evento_id">
							...
							<div v-show="esContingencia">
								...
								<div class="mb-2">
									<label>CUFD</label>
									<select class="form-control form-select no-select2" v-model="evento.cufd_evento">
	...
	Object.assign(SBFramework.AppComponents, {'siat-eventos': ns.ComEventos});
