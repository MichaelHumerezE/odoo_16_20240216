(function(ns)
{
	ns.ComSyncTiposEmision = {
		template: `<div id="com-sync-tipos-emision">
		...
		methods: 
		{
			...
			{
				try {
					const res = await this.$parent.service.obtenerTiposEmision(this.codigo_sucursal, this.codigo_puntoVenta);
					this.lista = res.data.RespuestaListaParametricas.listaCodigos;
				} catch (e) {
					this.$root.$processing.hide();
					alert(e.error || e.message || 'Error desconocido');
				}
				//const res = await this.$parent.service.obtenerTiposEmision();
				//this.lista = res.data.RespuestaListaParametricas.listaCodigos;
			}
		},
		created()
