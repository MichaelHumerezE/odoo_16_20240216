(function(ns)
{
	ns.ComSyncMetodosPago = {
		template: `<div id="com-sync-tipos-metodos-pago">
		...
		methods: 
		{
			...
			{
				try {
					const res = await this.$parent.service.obtenerMetodosPagoPiloto(this.codigo_sucursal, this.codigo_puntoVenta);
					this.lista = res.data.RespuestaListaParametricas.listaCodigos;
				} catch (e) {
					this.$root.$processing.hide();
					alert(e.error || e.message || 'Error desconocido');
				}
				//const res = await this.$parent.service.obtenerTiposMetodoPago();
				//this.lista = res.data.RespuestaListaParametricas.listaCodigos;
			}
		},
		created()
