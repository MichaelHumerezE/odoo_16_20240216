(function(ns)
{
	ns.ComInvoicer = {
												...
												<li v-for="(item, index) in customers_list" v-on:click="selectCustomer(item, index)" style="cursor:pointer;:hover:background:#ececec;padding:6px;border-bottom:1px solid #ececec;">
													<div class="container-fluid p-0 dropdown-item">
														<div class="row p-2 no-gutters">
															<div class="col-sm-2">
																<img v-bind:src="item.image ? item.image : '/images/client.png'" alt="" class="rounded-circle img-fluid img-thumbnail" />
															</div>
															<div class="col-sm-10">
				...
				<div class="modal-dialog">
					<div class="modal-content" novalidate>
						...
						<div class="modal-footer">
							...
							<a v-if="generatedInvoice" 
								v-bind:href="generatedInvoice.print_url_ticket"
								target="_blank" 
		...
		data()
		{
			return {
				...
				serviceProducts: new SBFramework.Services.ServiceProducts(),
			};
		},
		computed:
		{
		...
		methods: 
		{
			...
			async getMetodosPago()
			{
				...
				{
					if( [1,2,3,7].indexOf(pm.codigoClasificador) != -1 )
			...
			buscarProducto(by)
			{
				...
				this.searchProdTimeout = setTimeout(async () => 
				{
					try
					{
						const res = await this.serviceProducts.search(keyword);
	...
	Object.assign(SBFramework.AppComponents, {'siat-invoicer': ns.ComInvoicer});
