(function(ns)
{
	ns.ComSyncUnidadesMedida = {
		...
		methods: 
		{
			...
			{
				try {
					const res = await this.$parent.service.obtenerUnidadesMedidaPiloto(this.codigo_sucursal, this.codigo_puntoVenta);
					this.lista = res.data.RespuestaListaParametricas.listaCodigos;
				} catch (e) {
					this.$root.$processing.hide();
					alert(e.error || e.message || 'Error desconocido');
				}
				//const res = await this.$parent.service.obtenerUnidadesMedida();
				//this.lista = res.data.RespuestaListaParametricas.listaCodigos;
			}
		},
		created()
