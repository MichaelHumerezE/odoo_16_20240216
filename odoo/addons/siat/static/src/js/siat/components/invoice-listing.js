(function(ns)
{
	ns.InvoiceListing = {
		template: `<div id="com-siat-invoice-listing">
			...
			<div class="panel panel-default car border shadow" v-for="(invoice, index) in items">
				<div class="card-body">
					<div class="container-fluid">
						<div class="row">
							<div class="col-12 col-sm-7">
								<div>
									<span class="border rounded p-2 bg-primary text-white">ID: {{ invoice.invoice_id }}</span>
									<span class="border rounded p-2 bg-info text-white">Factura Nro: {{ invoice.invoice_number }}</span>
									<span class="border rounded p-2 bg-warning btn-warning text-white" v-if="invoice.evento_id">
								...
								<div>
									...
									|
									<b>Punto Venta:</b> {{ invoice.punto_venta }}
								</div>
							...
							<div class="col-12 col-sm-2">
								...
								<a v-bind:href="viewUrl(invoice, 'rollo')" target="_blank" class="btn btn-sm btn-warning w-100 mb-1 btn-block">Imprimir Ticket</a>
		...
		methods:
		{
			viewUrl(invoice, tpl)
			{
				return lt.baseurl + '/report/pdf/siat.siat_invoice_report'+ (tpl == 'rollo' ? '_ticket' : '') +'/'+ invoice.invoice_id;
			},
			changePage(page)
			{
			...
			async reenviar(invoice, index)
			{
				try
				{
					this.$root.$processing.show('Reenviando factura...');
					const res = await this.service.reenviar(invoice.invoice_id);
					this.$root.$processing.hide();
					this.$root.$toast.ShowSuccess(`Factura Nro. ${invoice.invoice_number} reeenviada correctamente`);
				}
				catch(e)
				{
					this.$root.$processing.hide();
	...
	Object.assign(SBFramework.AppComponents, {'siat-invoice-listing': ns.InvoiceListing});
