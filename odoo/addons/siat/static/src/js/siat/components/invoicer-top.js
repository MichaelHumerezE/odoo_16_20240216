	...
	ns.ComInvoicerTop = {
		template: `<div class="mb-1 shadow bg-white" style="padding:5px;border:1px solid #ececec;">
			<div class="row p-1">
				<div class="col-6 col-sm-3">
					...
					<a v-else class="btn btn-danger w-100" href="javascript:;" v-on:click="cerrarEvento">
		...
		methods: 
		{
			...
			async openEvent(eventoId, system)
			{
				...
				{
					this.$root.$processing.hide();
					this.$root.$toast.ShowError(e.error || e.message || 'Error desconocido al verificar eventos activos');
				}
			},
			...
			async cerrarEvento()
			{
				...
				{
					this.$root.$processing.hide();
					this.$root.$toast.ShowError(e.error || e.message || 'Ocurrio un error al cerrar el evento');
				}
			},
