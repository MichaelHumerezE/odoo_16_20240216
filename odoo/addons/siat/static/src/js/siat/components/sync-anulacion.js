...
{
	ns.ComSyncAnulacion = {
		template: `<div id="com-sync-anulacion">
		...
		methods: 
		{
			...
			{
				try {
					const res = await this.$parent.service.obtenerMotivosAnulacionPiloto(this.codigo_sucursal,this.codigo_puntoVenta);
					this.lista = res.data.RespuestaListaParametricas.listaCodigos;
				} catch (e) {
					this.$root.$processing.hide();
					alert(e.error || e.message || 'Error desconocido');
				}
				//const res = await this.$parent.service.obtenerMotivosAnulacion('/invoices/siat/v2/sync-motivos-anulacion');
				//this.lista = res.data.RespuestaListaParametricas.listaCodigos;
			}
		},
		created()
