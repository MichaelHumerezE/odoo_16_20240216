(function(ns)
{
	ns.ComSyncTiposMoneda = {
		template: `<div id="com-sync-tipos-moneda">
		...
		methods: 
		{
			...
			{
				try {
					const res = await this.$parent.service.obtenerTiposMoneda(this.codigo_sucursal, this.codigo_puntoVenta);
					this.lista = res.data.RespuestaListaParametricas.listaCodigos;
				} catch (e) {
					this.$root.$processing.hide();
					alert(e.error || e.message || 'Error desconocido');
				}
				//const res = await this.$parent.service.obtenerTiposMoneda();
				//this.lista = res.data.RespuestaListaParametricas.listaCodigos;
			}
		},
		created()
